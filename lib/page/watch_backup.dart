...
class _LoungePageState extends ConsumerState<WatchPage> {
  ...
  void dispatchPointerEvent(PointerEvent event) {
    // 押下ポインターイベント正確に送信
    // hitTest: イベントの位置に該当するウィジェットを正確に検出し、hitTestResultに格納します
    // イベントの伝播: ヒットテスト結果に基づいて、該当するウィジェットにイベントをディスパッチします。
                    ...
                    children: [
                      ...
                      Stack(
                        children: [
                          ...
                          GestureDetector(
                            onTapDown: (TapDownDetails details) { 
                              ...
                              // 1回目のタップのコールバック
                              print('1 isIntercepting == $isIntercepting');
                              isTapped = true;
                              isIntercepting = false;
                              print('2 isIntercepting == $isIntercepting');

                              // 1回目のタップ座標を取得
                              // globalPosition を Stack の相対位置に変換
                              RenderBox box = context.findRenderObject() as RenderBox;
                              firstTapPosition = box.globalToLocal(details.globalPosition);
                              ...
                              final pointerDownEvent = PointerDownEvent(
                                position: firstTapPosition!,
                                kind: PointerDeviceKind.mouse,
                                pointer: pointerID, // 一意のポインターIDを設定
                              );
                              ...
                              final pointerUpEvent = PointerUpEvent(
                                position: firstTapPosition!,
                                kind: PointerDeviceKind.mouse,
                                pointer: pointerID, // 同一のポインターIDを使用
                              );

                              // ヒットテスト結果オブジェクトを作成
                              final hitTestResult = HitTestResult();

                              print('Pointer down event position: ${pointerDownEvent.position}');
                              print('Pointer up event position: ${pointerUpEvent.position}');
                              print('4');


                              dispatchPointerEvent(pointerDownEvent);
                              dispatchPointerEvent(pointerUpEvent);

                              setState(() {
                                isIntercepting = false;  
                              });
                              print('5 isIntercepting == $isIntercepting');
                        ...
                        if (firstTapPosition != null)
                          Positioned(
                            left: firstTapPosition!.dx - 5, // 円の中央に位置合わせ
                            top: firstTapPosition!.dy - 5,
                            child: Container(
                              width: 10,
                              height: 10,
                              decoration: const BoxDecoration(
